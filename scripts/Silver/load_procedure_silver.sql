...
Script Purpose:
    ...
    - Loads standardized data into Silver tables
    - Captures execution duration and error handling

Parameters:
    None.
    This stored procedure does not accept input parameters
    or return explicit values.

Usage:
    Run this stored procedure to refresh the Silver layer
    with the latest transformed data from Bronze.
...
BEGIN
	...
	SELECT
				...
				CASE 
					...
					ELSE 'n/a'
				END AS gndr, -- Normalize gender values to readable format
				cst_create_date
			FROM (
				SELECT
					*,
...
PRINT' - TOTAL LOAD DURATIONS: ' +CAST (DATEDIFF (SECOND, @batch_start_time,@batch_end_time) AS NVARCHAR)+ 'seconds';
