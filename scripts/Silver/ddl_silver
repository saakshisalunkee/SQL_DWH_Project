...
Layer       : Silver
    ...
    Existing tables are dropped first if they already exist.

    Silver Layer Purpose:
    - Store cleaned and transformed data from bronze layer
    - Apply correct datatypes
    - Standardize values
    - Add ETL metadata (load timestamp)
...
CREATE TABLE silver.crm_prd_info (
    ...
    prd_start_dt        DATE,
    prd_end_dt          DATE,             -- generated using LEAD()
    dwh_create_date     DATETIME2 DEFAULT GETDATE()
);
GO
